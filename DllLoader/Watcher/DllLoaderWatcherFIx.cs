...
namespace Oxide.Ext.DllLoader.Watcher
{
    internal class DllLoaderWatcherFix
    {
        ...
        static DllLoaderWatcherFix()
        {
            ...
            WatcherAddMappingPatchInfo = new HarmonyMethod(dllWatcherFixType.GetMethod(nameof(WatcherAddMapping), staticPrivateBinds));
            WatcherRemoveMappingPatchInfo = new HarmonyMethod(dllWatcherFixType.GetMethod(nameof(WatcherRemoveMapping), staticPrivateBinds));
            ...
            FirePluginSourceChangedInfo = pluginChangeWatcherType.GetMethod("FirePluginSourceChanged", instancePrivateBinds);

            FirePluginAddedPatchInfo = new HarmonyMethod(dllWatcherFixType.GetMethod(nameof(FirePluginAdded), staticPrivateBinds));
            FirePluginRemovedPatchInfo = new HarmonyMethod(dllWatcherFixType.GetMethod(nameof(FirePluginRemoved), staticPrivateBinds));
            FirePluginSourceChangedPatchInfo = new HarmonyMethod(dllWatcherFixType.GetMethod(nameof(FirePluginSourceChanged), staticPrivateBinds));

            OnPluginAddedEvent = pluginChangeWatcherType.GetField(nameof(PluginChangeWatcher.OnPluginAdded), instancePrivateBinds);
            OnPluginRemovedEvent = pluginChangeWatcherType.GetField(nameof(PluginChangeWatcher.OnPluginRemoved), instancePrivateBinds);
            OnPluginSourceChangedEvent = pluginChangeWatcherType.GetField(nameof(PluginChangeWatcher.OnPluginSourceChanged), instancePrivateBinds);
        }
        #endregion

        private static readonly IDictionary<PluginChangeWatcher, IDllLoaderMapper> _fsWatcherMappers = new Dictionary<PluginChangeWatcher, IDllLoaderMapper>();

        #region PluginWatcherColletion
        private static bool MapPluginNameToAssembly(PluginChangeWatcher __instance, string pluginName, Action<AssemblyInfo> action)
        {
        ...
        private static bool MapAssemblyNameToEvent(PluginChangeWatcher __instance, string assemblyName, FieldInfo fieldInfo)
        {
            if (!_fsWatcherMappers.TryGetValue(__instance, out var mapper))
            {
                Interface.Oxide.LogDebug("No corresponding watcher found, ignoring...");
